...
This `Build Docker App Image` workflow is triggered `on: pull_request` of `types: closed` on `branches: feature*` whenever any of `paths: models/*.pkl.dvc` changes. It is triggered by an automatic merge PR (pull_request) action that is generated by a `github-actions[bot]` running at the end of the Step 3 `Train ML Model` workflow [Generate Pull request](https://github.com/peter-evans/create-pull-request). Alternatively this workflow may also be triggered by manually running in from the GitHub Actions menu tab.
